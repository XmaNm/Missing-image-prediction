...
class HouseholderTransformation(nn.Module):
    def __init__(self, in_dim, out_dim, hps):
        """

        :param in_dim: number of input  features
        :param out_dim: number of output features
        :param hps: the set of hyperparameters.
        """
        self.in_dim = hps.in_dim
        self.out_dim = hps.out_dim

    def HF(self, args):
        """
        compute the householder matrix of target.
        :param args:
        :return:
        """
        z_v, z = args
        A = torch.bmm(torch.unsqueeze(z_v, 2), torch.unsqueeze(z_v, 1))
        Az = torch.reshape(torch.bmm(A, torch.unsqueeze(z, -1)),(-1, 1024))
        v_norm_sq = torch.sum(torch.square(z_v),keepdim=True)
